...
class Retinex:
    ...
    def gau_process(self,I):
        ...
        Ir = img[...,0]
        sigma = [15,80,250]
        x, y = np.meshgrid(range(int((-(np.shape(Ir)[1]-1)/2)), int((np.shape(Ir)[1])/2)), range(int((-(np.shape(Ir)[0]-1)/2)), int((np.shape(Ir)[0])/2)))
...
plt.show()
#calıstı ve retınex algorıtması olarak kaydetttım.